...
class FringeAnalysis:
	...
	@staticmethod
	def bin_array(numpy_array, threshold=128):
		"""Bin a numpy array."""
	...
	@staticmethod
	def find_curvature(image, start):
		dis = 0
		x = start[0]
		y = start[1]
		end = 1
	...
	def process_lc(self):
		self.filter_sizes()
		labels = measure.label(self.image_bw, background=0)  # http://scikit-image.org/docs/dev/api/skimage.measure.html
		final_label = np.zeros((len(labels), len(labels[0])))
		print(len(final_label), len(final_label[0]))
		df = pd.DataFrame(columns=['image', 'coord_start', 'coord_end', 'c', 'l'])
		...
		pb = ProgressBar(total=bar, prefix='processed fringes', suffix='Done', decimals=3, length=50, fill='X', zfill='-')
		for region in a:
			...
			if (k == 2) & (len(alpha) != 0) & (len(omega) != 0):
				long = mt.sqrt(mt.pow((alpha[0]-omega[0]), 2) + mt.pow((alpha[1]-omega[1]), 2))
				curvature = self.find_curvature(aux, alpha)
				df = df.append(
